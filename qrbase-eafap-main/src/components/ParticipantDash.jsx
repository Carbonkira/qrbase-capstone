...
const ParticipantDash = () => {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#e9eff6] font-sans text-slate-800 overflow-hidden relative">
                ...
                <div key={ticket.id} className="bg-white p-8 rounded-[3rem] shadow-sm hover:shadow-2xl transition-all cursor-pointer border-4 border-transparent hover:border-blue-100 flex flex-col h-full" onClick={() => openTicket(ticket)}>
                    ...
                    <div className="h-48 w-full bg-slate-100 rounded-[2rem] mb-6 overflow-hidden relative shadow-inner">
                        ...
                        <div className="absolute top-4 right-4">
                            <span className={`text-xs font-black uppercase tracking-widest px-4 py-2 rounded-xl shadow-sm ${
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
              <div className="bg-white w-full max-w-sm rounded-[2.5rem] shadow-2xl p-10 animate-in zoom-in">
                  <div className="flex justify-between items-center mb-6">
                      <h3 className="text-2xl font-black text-[#1e40af] uppercase text-center flex-1">{isPasswordMode ? "Change Password" : "Edit Profile"}</h3>
                  </div>
                  <div className="flex bg-slate-100 p-1 rounded-xl mb-6">
                      <button onClick={() => setIsPasswordMode(false)} className={`flex-1 py-2 rounded-lg text-xs font-black uppercase transition-all ${!isPasswordMode ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}>Profile</button>
                      <button onClick={() => setIsPasswordMode(true)} className={`flex-1 py-2 rounded-lg text-xs font-black uppercase transition-all ${isPasswordMode ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}>Security</button>
                  </div>
                  ...
                  {isPasswordMode ? (
                      <form onSubmit={handleChangePassword} className="flex flex-col gap-4">
                          ...
                          <div className="flex gap-2 mt-4"><button type="button" onClick={() => setIsProfileOpen(false)} className="flex-1 bg-slate-100 text-slate-500 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-200">Cancel</button><button type="submit" className="flex-1 bg-[#1e293b] text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-blue-600">Update</button></div>
                      </form>
                  ) : (
                      <form onSubmit={handleUpdateProfile} className="flex flex-col gap-4">
                          ...
                          <div className="flex gap-2 mt-4"><button type="button" onClick={() => setIsProfileOpen(false)} className="flex-1 bg-slate-100 text-slate-500 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-200">Cancel</button><button type="submit" className="flex-1 bg-[#1e293b] text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-blue-600">Save</button></div>
                      </form>
                  )}
              </div>
          </div>
      ...
      {/* TICKET DETAILS MODAL (WITH SPEAKER BIOS & EVENT DESCRIPTION) */}
      {selectedEvent && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
            <div className="bg-white w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-[2.5rem] shadow-2xl p-8 animate-in zoom-in relative flex flex-col md:flex-row gap-8">
                ...
                <div className="flex flex-col items-center justify-start bg-blue-50 p-8 rounded-[2rem] min-w-[260px] h-fit">
                    {(selectedEvent.payment_status === 'Paid' || selectedEvent.payment_status === 'Free') ? (
                        <>
                            ...
                            <div className="text-center">
                                ...
                                <div className="bg-blue-100/50 p-2 rounded-lg inline-block max-w-[200px]">
                        ...
                        <div className="bg-orange-100 p-6 rounded-2xl text-center border-2 border-orange-200">
                             <h3 className="text-xl font-black text-orange-600 uppercase mb-2">Payment Pending</h3>
                             <p className="text-xs font-bold text-orange-400">Please verify your payment with the organizer to reveal your QR Code.</p>
                        </div>
                    )}
                    
                    {selectedEvent.status === 'Waitlisted' && (
                        <div className="mt-4 bg-red-100 text-red-500 px-4 py-2 rounded-xl text-xs font-black uppercase w-full text-center">Waitlisted</div>
                    )}
                </div>

                {/* RIGHT COLUMN: EVENT & SPEAKER DETAILS */}
                <div className="flex-1 flex flex-col min-w-0">
                    <div className="h-32 w-full bg-slate-100 rounded-[1.5rem] mb-6 overflow-hidden shrink-0">
                        {selectedEvent.event.image ? <img src={STORAGE_URL + selectedEvent.event.image} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-300 font-bold text-xs uppercase tracking-widest">No Image</div>}
                    </div>
                    <h2 className="text-3xl font-black text-[#1e40af] uppercase leading-tight mb-4">{selectedEvent.event.title}</h2>
                        ...
                        <div className="border-t-2 border-slate-100 pt-6">
                            ...
                            <div className="space-y-4">
                                {selectedEvent.event.speakers.map(s => (
                                    <div key={s.id} className="bg-slate-50 border border-slate-100 p-5 rounded-[1.5rem] flex gap-4 items-start">
                                        <div className="w-14 h-14 bg-slate-200 rounded-full overflow-hidden shrink-0 flex items-center justify-center text-blue-500 font-black text-xl">
                                            {s.photo_path ? <img src={STORAGE_URL + s.photo_path} alt={s.name} className="w-full h-full object-cover" /> : s.name.charAt(0)}
                                        </div>
                                        <div className="flex-1">
                                            <div className="flex justify-between items-start mb-1">
                                                <h4 className="font-black text-slate-700 uppercase">{s.name}</h4>
                                                <span className="bg-blue-100 text-blue-600 px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider">{s.pivot?.topic || s.topic || "Speaker"}</span>
                                            </div>
                                            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">{s.specialization}</p>
                                            {s.contact_email && <p className="text-xs text-blue-500 font-bold mb-2">Contact: {s.contact_email}</p>}
                                            <p className="text-sm text-slate-500 leading-relaxed bg-white p-3 rounded-xl border border-slate-100">{s.description || "No bio provided."}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
