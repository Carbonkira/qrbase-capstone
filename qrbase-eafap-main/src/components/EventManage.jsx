...
const EventManage = () => {
  ...
  const handleDeleteSpeaker = async (speakerId) => {
      if(confirm("Remove this speaker from the event?")) { await api.delete(`/speakers/${speakerId}`); fetchData(); }
  };
  ...
  const getQuestionDetails = (key) => { const parts = key.split('_'); if (parts[0] === 'global') { const index = parseInt(parts[1]); return { label: evalConfig.global[index]?.text || "Unknown Question", type: evalConfig.global[index]?.type }; } if (parts[0] === 'speaker') { const speakerId = parts[1]; const index = parseInt(parts[2]); const speakerName = speakers.find(s => s.id == speakerId)?.name || "Unknown Speaker"; const qText = evalConfig.speakers[speakerId]?.[index]?.text || "Unknown Question"; return { label: `[${speakerName}] ${qText}`, type: evalConfig.speakers[speakerId]?.[index]?.type }; } if (key === 'final_comments') return { label: "Final Comments", type: 'text' }; return { label: key, type: 'text' }; };
  const calculateStats = () => { if (!data || !data.attendees) return []; const feedbacks = data.attendees.map(a => a.feedback).filter(f => f); if (feedbacks.length === 0) return null; const stats = []; evalConfig.global.forEach((q, idx) => { if (q.type === 'rating') { const key = `global_${idx}`; const ratings = feedbacks.map(f => f[key]).filter(r => r > 0); const avg = ratings.length ? (ratings.reduce((a, b) => a + Number(b), 0) / ratings.length).toFixed(1) : 0; stats.push({ label: q.text, value: avg, type: 'rating' }); } }); Object.keys(evalConfig.speakers).forEach(sid => { evalConfig.speakers[sid].forEach((q, idx) => { if (q.type === 'rating') { const key = `speaker_${sid}_${idx}`; const ratings = feedbacks.map(f => f[key]).filter(r => r > 0); const avg = ratings.length ? (ratings.reduce((a, b) => a + Number(b), 0) / ratings.length).toFixed(1) : 0; const speakerName = data.speakers.find(s => s.id == sid)?.name; stats.push({ label: `[${speakerName}] ${q.text}`, value: avg, type: 'rating' }); } }); }); return stats; };
  const feedbackStats = calculateStats();

  if (loading) return <div className="p-10 text-center font-black text-slate-400 text-xl">LOADING MODULE...</div>;
  if (!data) return <div className="p-10 text-center text-red-400 font-bold text-xl">Event Not Found</div>;

  const { event, stats, attendees, speakers } = data;
  ...
  return (
    <div className="h-screen flex flex-col bg-[#e9eff6] font-sans text-slate-800 relative">
      ...
      <header className="flex justify-between items-center px-12 py-6 bg-white shadow-sm sticky top-0 z-50 shrink-0">
        <div>
            <button onClick={() => navigate('/events')} className="text-xs font-black text-slate-400 uppercase tracking-widest hover:text-blue-600 mb-1">← Back to Events</button>
            <h1 className="text-4xl font-black text-[#1e40af] uppercase tracking-tight">{event.title}</h1>
            <div className="mt-2 flex items-center gap-2">
                <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Invite Code:</span>
                <span className="text-lg font-black text-blue-600 bg-blue-50 px-3 py-1 rounded-lg border border-blue-100">{event.invite_code}</span>
            </div>
        </div>
        <div className="flex items-center gap-6">
            <div className="flex gap-4">
                {['Overview', 'Attendance', 'Speakers', 'Evaluation', 'Scanner'].map(tab => (
                    <button key={tab} onClick={() => setActiveTab(tab)} className={`px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-[#1e293b] text-white shadow-lg' : 'bg-slate-100 text-slate-400 hover:bg-white'}`}>{tab}</button>
                ))}
            </div>
            ...
            {user && (
                <div onClick={() => setIsProfileOpen(true)} className="flex items-center gap-4 text-right cursor-pointer hover:opacity-80 transition-opacity group">
                    <div className="hidden sm:block"><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest group-hover:text-blue-500">Edit Profile</p><p className="text-lg font-black text-[#1e40af] uppercase leading-none">{user.first_name} {user.last_name}</p></div>
      ...
      <div className="flex-1 overflow-y-auto p-8">
        <div className="max-w-7xl mx-auto">
            ...
            {activeTab === 'Overview' && (
                 <div className="space-y-10">
                     ...
                     <div className="grid grid-cols-4 gap-8">
                        <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-white/50 flex flex-col items-center"><span className="text-6xl font-black text-[#1e40af] mb-2">{stats.total_registered}</span><span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Registered</span></div>
                        <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-white/50 flex flex-col items-center"><span className="text-6xl font-black text-green-600 mb-2">{stats.total_present}</span><span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Present</span></div>
                        <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-white/50 flex flex-col items-center"><span className="text-6xl font-black text-purple-600 mb-2">{stats.feedback_received}</span><span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Feedbacks</span></div>
                        <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-white/50 flex flex-col items-center"><span className="text-6xl font-black text-orange-500 mb-2">{stats.capacity - stats.total_registered}</span><span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Slots Left</span></div>
                     </div>
                 </div>
            )}
            ...
            {activeTab === 'Attendance' && (
                <div className="bg-white rounded-[3rem] shadow-xl p-10">
                    <div className="flex justify-between items-center mb-8">
                        <h3 className="font-black text-2xl text-[#1e40af] uppercase">Participant List</h3>
                        <button onClick={handleExportCSV} className="bg-green-50 text-green-600 px-6 py-3 rounded-2xl font-black text-xs uppercase hover:bg-green-100 flex items-center gap-2"><Icon path="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />Download CSV Report</button>
                    </div>
                    <table className="w-full text-left">
                        <thead><tr className="border-b-2 border-slate-100"><th className="p-5 text-xs font-black text-slate-400 uppercase">Participant</th><th className="p-5 text-xs font-black text-slate-400 uppercase">Status</th><th className="p-5 text-xs font-black text-slate-400 uppercase text-right">Feedback</th><th className="p-5 text-xs font-black text-slate-400 uppercase text-right">Actions</th></tr></thead>
                        <tbody>
                            {attendees.map(u => (
                                <tr key={u.id} className="border-b border-slate-50 hover:bg-slate-50">
                                    <td className="p-5 font-bold text-slate-700 text-sm">{u.user.first_name} {u.user.last_name}<div className="text-xs text-slate-400 font-medium">{u.user.email}</div></td>
                                    <td className="p-5"><span className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase ${u.status === 'Present' ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-500'}`}>{u.status}</span></td>
                                    <td className="p-5 text-right">
                                        {u.feedback ? <button onClick={() => setSelectedFeedbackUser(u)} className="bg-purple-50 text-purple-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-purple-100 border border-purple-100">View Answers</button> : <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">Pending</span>}
                                    </td>
                                    <td className="p-5 flex justify-end gap-3"><button onClick={() => handleUpdateStatus(u.id, 'Present')} className="text-[10px] font-bold text-green-600 bg-green-50 px-4 py-2 rounded-xl hover:bg-green-100">PRESENT</button><button onClick={() => handleUpdateStatus(u.id, 'Absent')} className="text-[10px] font-bold text-red-600 bg-red-50 px-4 py-2 rounded-xl hover:bg-red-100">ABSENT</button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                     ...
                     {speakers.map(s => (
                         <div key={s.id} onClick={() => setSelectedSpeaker(s)} className={`bg-white p-8 rounded-[3rem] shadow-lg flex gap-6 items-center border relative group cursor-pointer transition-all ${speakerForm.id === s.id ? 'border-blue-400 ring-4 ring-blue-100' : 'border-slate-50 hover:border-blue-200'}`}>
                             ...
                             <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest truncate mb-2">{s.specialization}</p>
                                <div className="bg-blue-50 px-3 py-1 rounded-lg inline-block border border-blue-100">
                                    <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest truncate max-w-[120px]">Topic: {s.pivot?.topic || s.topic || "TBA"}</p>
                                </div>
                             </div>
                         </div>
                     ))}
            ...
            {/* EVALUATION, SCANNER & MODALS (Keep existing code from previous responses) */}
            {/* EVALUATION TAB */}
                ...
                <div className="space-y-8">
                    {isFormLocked ? (
                        <div className="bg-white rounded-[3rem] shadow-xl p-12 text-center">
                            ...
                            <h3 className="text-3xl font-black text-slate-800 uppercase mb-4">Form is Active</h3>
                            ...
                            <div className="bg-slate-50 p-8 rounded-[2rem] text-left max-w-2xl mx-auto border border-slate-100">
                                <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6 border-b border-slate-200 pb-3">Configured Questions</h4>
                        ...
                        <>
                            <div className="bg-white rounded-[3rem] shadow-xl p-10">
                                <div className="flex justify-between items-center mb-8 border-b border-slate-100 pb-6">
                                    <div><h3 className="font-black text-2xl text-[#1e40af] uppercase">General Event Questions</h3></div>
                                    <button onClick={() => addQuestion('global')} className="bg-blue-50 text-blue-600 px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-blue-100">+ Add</button>
                                </div>
                                <div className="space-y-4">
                                    {evalConfig.global.map((q, idx) => (
                                        <div key={idx} className="flex gap-4 items-center bg-[#f8fafc] p-4 rounded-2xl border border-slate-100">
                                            <select value={q.type} onChange={(e) => updateQuestion('global', idx, 'type', e.target.value)} className="bg-white border border-slate-200 rounded-xl text-[10px] font-black uppercase p-3 outline-none"><option value="rating">Rating (1-5)</option><option value="text">Text Answer</option></select>
                                            ...
                                            <button onClick={() => removeQuestion('global', idx)} className="text-red-300 hover:text-red-500 font-black px-3">X</button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            <div className="bg-white rounded-[3rem] shadow-xl p-10">
                                <div className="mb-8 border-b border-slate-100 pb-6"><h3 className="font-black text-2xl text-[#1e40af] uppercase">Speaker Specific Questions</h3></div>
                                <div className="grid grid-cols-1 gap-8">
                                    {speakers.map(s => (
                                        <div key={s.id} className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
                                            <div className="flex justify-between items-center mb-6">
                                                <div className="flex flex-col">
                                                    <span className="font-black text-lg text-slate-700 uppercase">{s.name}</span>
                                                    <span className="text-[10px] font-bold text-blue-500 uppercase tracking-widest">Topic: {s.pivot?.topic || s.topic || "TBA"}</span>
                                                </div>
                                                <button onClick={() => addQuestion('speaker', s.id)} className="bg-white text-slate-400 px-4 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest border border-slate-200">+ Add</button>
                                            </div>
                                            <div className="space-y-3">
                                                {(evalConfig.speakers[s.id] || []).map((q, idx) => (
                                                    <div key={idx} className="flex gap-4 items-center bg-white p-3 rounded-2xl border border-slate-200">
                                                        <select value={q.type} onChange={(e) => updateQuestion('speaker', idx, 'type', e.target.value, s.id)} className="bg-slate-50 border border-slate-200 rounded-xl text-[10px] font-black uppercase p-3 outline-none"><option value="rating">Rating</option><option value="text">Text</option></select>
                                                        ...
                                                        <button onClick={() => removeQuestion('speaker', idx, s.id)} className="text-red-300 hover:text-red-500 font-black px-3">X</button>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                            ...
                            <button onClick={handleSaveForm} className="w-full bg-[#1e293b] text-white py-5 rounded-[2rem] font-black text-sm uppercase tracking-widest hover:bg-blue-800 shadow-xl transition-all active:scale-95">Save & Lock Configuration</button>
                        </>
                    )}
                </div>
            )}
            ...
            {activeTab === 'Scanner' && (
                 ...
                 <h3 className="text-3xl font-black text-[#1e40af] uppercase mb-10">Ticket Scanner</h3>
                 {!scanResult ? (
                    <div className="flex flex-col items-center w-full max-w-md gap-10">
                        ...
                        <div className="w-full h-px bg-slate-100 relative my-2"><span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-4 text-xs font-black text-slate-300 uppercase tracking-widest">OR ENTER MANUALLY</span></div>
                        <form onSubmit={handleManualSubmit} className="flex w-full gap-3">
                            ...
                            <button type="submit" className="bg-[#1e293b] text-white px-8 rounded-2xl font-black uppercase text-xs">Check In</button>
                        </form>
                    </div>
                ) : (
                    <div className={`p-10 rounded-[2.5rem] text-center w-full max-w-sm animate-in zoom-in ${scanResult.success ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}>
                        ...
                        <h4 className="text-3xl font-black uppercase mb-3">{scanResult.success ? "Verified!" : "Error"}</h4>
                        <p className="font-bold text-lg opacity-80">{scanResult.message}</p>
                        {scanResult.user && <div className="mt-6 bg-white/50 p-6 rounded-2xl"><p className="text-xs font-black uppercase tracking-widest opacity-50">Participant</p><p className="text-xl font-bold">{scanResult.user.first_name} {scanResult.user.last_name}</p></div>}
                        <button onClick={resetScan} className="mt-10 bg-[#1e293b] text-white w-full py-5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-black shadow-lg">Scan Next</button>
                    </div>
                )}
            </div>
            )}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
                    <div className="bg-white w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-[3rem] shadow-2xl p-10 animate-in zoom-in relative">
                        ...
                        <div className="text-center mb-10">
                            <h3 className="text-2xl font-black text-[#1e40af] uppercase">Participant Feedback</h3>
                            ...
                            {Object.entries(selectedFeedbackUser.feedback || {}).map(([key, answer]) => {
                                ...
                                return (
                                    <div key={key} className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{details.label}</p>
                        ...
                        <div className="flex flex-col items-center text-center">
                            <div className="w-32 h-32 bg-slate-100 rounded-full mb-8 overflow-hidden border-4 border-white shadow-lg">
                                {selectedSpeaker.photo_path ? (
                                    ...
                                    <div className="w-full h-full flex items-center justify-center text-slate-300 font-black text-4xl">{selectedSpeaker.name.charAt(0)}</div>
                                )}
                            </div>
